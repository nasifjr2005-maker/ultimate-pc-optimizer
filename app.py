...
from core.system import (
    clean_temp_files, cpu_summary, empty_recycle_bin, flush_dns, is_admin,
    list_background_processes, ram_summary, set_high_performance_power_plan,
    terminate_process,
)
...
class Dashboard(QMainWindow):
    ...
    def setup_ui(self):
        ...
        nv.addSpacing(20)
        for text, index in [
            ("Dashboard", 0), ("PC Cleanup", 1), ("Gaming Mode", 2),
            ("Emulators", 3), ("Background Apps", 4),
        ]:
            b = QPushButton(text)
    ...
    def emulator_page(self):
        w, lay = self.page_base(
            "Emulator Optimizer",
            "Detect BlueStacks / MSI App Player (HD-Player.exe), raise emulator process priority, use all logical CPUs, and keep the app's ADB target at local port 5555.",
        )
        self.emu_list = QListWidget()
        lay.addWidget(self.emu_list)
        row = QHBoxLayout()
        scan = QPushButton("SCAN EMULATORS")
        scan.clicked.connect(self.refresh_emulators)
        opt = QPushButton("OPTIMIZE SELECTED")
        opt.clicked.connect(self.optimize_selected_emulator)
        adb = QPushButton("ADB CONNECT :5555")
        adb.clicked.connect(self.do_adb)
        row.addWidget(scan)
        row.addWidget(opt)
        row.addWidget(adb)
        lay.addLayout(row)
    ...
    def process_page(self):
        w, lay = self.page_base(
            "Background Apps",
            "Select user applications you no longer need right now. Critical Windows/system processes are not presented for one-click termination.",
        )
    ...
    def do_cleanup(self):
        ...
        self.cleanup_status.setText(
            f"Removed {result['files']} files and {result['dirs']} folders. Locked/protected items were skipped."
        )
        self.refresh_dashboard()
    ...
    def refresh_emulators(self):
        ...
        self.adb_status.setText(
            f"ADB target: 127.0.0.1:{default_adb_port()} • "
            f"{'OPEN' if port_is_open() else 'NOT LISTENING'}"
        )
    ...
    def close_selected_processes(self):
        ...
        for i in range(self.proc_list.count()):
            ...
            if item.checkState() == Qt.Checked:
                ok, msg = terminate_process(int(item.data(Qt.UserRole)))
                msgs.append(msg)
        self.refresh_processes()
